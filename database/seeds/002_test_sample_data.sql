...
-- Idempotent: guarded by an early RETURN so re-running `npm run db:seed`
-- never duplicates the opportunity (the e2e test mutates it, but a plain
-- re-seed on an unmodified database is always a no-op here).
...
DECLARE
  ...
  v_rfq_id UUID;
BEGIN
  IF EXISTS (SELECT 1 FROM opportunities WHERE listing_url = 'https://example-test-listing.invalid/TEST-SITRAK-C7H-001') THEN
    RETURN;
  END IF;
  ...
  VALUES (v_supplier_id, 'TEST Contact Zhang', 'Sales Manager', '+86-000-0000-0000', 'contact@example-test-supplier.invalid', true);
  ...
  VALUES (v_supplier_id, 'TEST Shandong Sample Trucks Co., Ltd.', false, 'TEST Bank of China', 'TEST0000000000001', true, 'VERIFIED');
  ...
  INSERT INTO opportunities (
    ...
    v_supplier_id, 'MANUAL', 'https://example-test-listing.invalid/TEST-SITRAK-C7H-001',
    'SITRAK', 'C7H', '8x4', 2021,
  ...
  VALUES (v_opportunity_id, 'TESTVINCN0000001', 'UNKNOWN', 'MC13', 'UNKNOWN', 'HW19710', '["chassis_number","engine_serial","gearbox_serial","ecu_reference"]', 'DRAFT');

  -- Sample RFQ + quote
  INSERT INTO rfqs (opportunity_id, supplier_id, status, sent_at)
  VALUES (v_opportunity_id, v_supplier_id, 'SENT', now() - interval '10 days')
  ...
  INSERT INTO quotes (
    ...
    v_rfq_id, v_opportunity_id, v_supplier_id, 17800, 600, 450, 300, 19500, 24800, 250,
    'USD', true, 'RANKED'
  );

  -- Sample inspection
  INSERT INTO inspections (opportunity_id, inspector_name, inspection_date, mechanical_score, recommendation, status)
  VALUES (v_opportunity_id, 'TEST Inspector', CURRENT_DATE, 89, 'PASS', 'COMPLETE');
END $$;
